...
main() {
  group('getCocktailDetails', () {
    test('returns Cocktail if the http call completes successfully', () async {
      ...
      expect(
          await repository.fetchCocktailDetails('0', client), isA<Cocktail>());
    });

    test('throws an exception if the http call completes with an error', () {
      final client = MockClient();

      when(client.get('https://the-cocktail-db.p.rapidapi.com/lookup.php?i=0'))
      ...
      when(client.get(
        ...
        headers: anyNamed("headers"),
          ...
          (Invocation _) => Future.value(http.Response('Not Found', 200)));

      expect(repository.fetchCocktailDetails('0', client), throwsException);
    });
  });
}
